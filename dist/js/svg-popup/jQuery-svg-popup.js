		...
		var defaults = {
			...
			createMarkup: function(){
				var posX = config.width/config.stepX,
					  posY = config.height/config.stepY,					
						html = '<svg class="at-particles-wrap">';  
			...
			animPolygon: function(){
				...
				setTimeout(function(){
					popup.content.css({
						'top': '50%'
					}).append('<span class="at-close">'+ config.closeButtonText +'</span>');
					$('.at-close').on('click', function(){
						popup.destroyPolygon($(this));
					});
					console.log(config.onAnimationComplate(that));
					config.onAnimationComplate(that);
				},config.speed*1000);
			},
			init: function(){				
				that.append(popup.createMarkup());
				this.svg     = $('svg.at-particles-wrap');
				this.polygon = this.svg.find('polygon');
				this.content = that.find('.at-content');
				this.content.css({
					'position': 'fixed',								
					'left': '50%',
					'top': '110%',
				...
				that.find('.at-button').on('click', function(){				
					popup.animPolygon();
				});
			}
		};
